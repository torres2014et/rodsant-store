...
- **Panel Filament v4:** Dashboard (widgets `StatsOverview`, `LatestOrders`, `TopProducts`, **`SalesChart`** — gráfica de barras de ventas con selector semanal/mensual; periodos calculados en PHP para servir igual en MySQL y sqlite) + **Productos** (`ProductResource`, form slug-auto, imágenes reordenables, SEO, `VariantsRelationManager`) + **Pedidos** (`OrderResource`, grupo "Ventas": lista con cambio de estado en línea vía `SelectColumn`, página Ver con tabla de artículos+totales renderizada en `OrderForm::itemsTable`, edición de estado/pago/notas, insignia de pendientes; sin crear a mano) + **Inventario** (`InventoryResource`, slug `inventory`, ajuste de stock en línea vía `TextInputColumn`, `reserved` solo lectura, filtros agotado/bajo stock, insignia naranja) + **Categorías** (`CategoryResource`, slug-auto, subcategorías con `parent_id` que excluye la propia, imagen, género, orden, SoftDeletes) + **Configuración** (`app/Filament/Pages/StoreSettings.php`, gated `settings.manage`: número WhatsApp, contacto, redes).
...
- **41 tests passing** (Home, Catalog/PDP, CheckoutFlow, CurrencySwitcher, AdminPanel —incluye Pedidos/Inventario/Categorías—, StoreSettings).

**Pendiente (módulos del panel):** Clientes · Banners · Usuarios. (Configuración: solo WhatsApp/contacto/redes; falta el resto de ajustes.) **Hecho:** Pedidos · Inventario · Categorías.
